...
const Tasks = () => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    // Apply search filter
    if (sidebarFilters.searchTerm.trim()) {
      tasksToFilter = tasksToFilter.filter(task =>
        task.title.toLowerCase().includes(sidebarFilters.searchTerm.toLowerCase())
      );
    }
  ...
  useEffect(() => {
    setSelectedProjectFilter(sidebarFilters.projectId === 'all' ? null : sidebarFilters.projectId);
    setSelectedPriorityFilter(sidebarFilters.priorities.length > 0 ? sidebarFilters.priorities[0] : null);
    ...
    <AppLayout
      contextualSidebar={
        <TaskFiltersSidebar
          ...
          users={allUsers}
          canFilterByUser={permissions.isAdmin}
          canShowMyReviews={permissions.isAdmin || permissions.isManager}
          isLoading={isLoading}
          title="Tarefas"
          subtitle="Visualize e gerencie todas as suas tarefas."
        />
      }
    >
      ...
      <div className="-mx-4 md:-mx-6 h-full">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 ml-auto">
                    {permissions.can('task:create') && !permissions.isMember && (
                      <Button variant="outline" size="sm" className="gap-1" disabled={isLoading} onClick={() => setIsRecurringTasksDialogOpen(true)}>
                        ...
                        <span className="hidden lg:inline">Recorrentes</span>
                      </Button>
                    )}
                    {permissions.can('task:create') && !permissions.isMember && (
                      <Button size="sm" className="gap-1" disabled={isLoading} onClick={() => {
