...
export default class RecurringTasksController {
    ...
    const tasksWithOccupations = await Promise.all(
      recurringTasks.map(async (task) => {
        ...
        if (taskJson.templateData.occupation_ids && taskJson.templateData.occupation_ids.length > 0) {
          const occupations = await Occupation.query().whereIn('id', taskJson.templateData.occupation_ids)
  ...
  async store({ request, response }: HttpContext) {
    ...
    const recurringTask = await RecurringTask.create({
      ...
      next_due_date: payload.next_due_date ? DateTime.fromISO(payload.next_due_date) : DateTime.now(),
  ...
  async show({ params, response }: HttpContext) {
    const recurringTask = await RecurringTask.query().where('id', params.id).preload('user').firstOrFail()
