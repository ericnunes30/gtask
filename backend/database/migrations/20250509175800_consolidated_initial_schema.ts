...
const TaskStatus = { Backlog: 'pendente', ToDo: 'a_fazer', InProgress: 'em_andamento', Review: 'em_revisao', WaitingClient: 'aguardando_cliente', Done: 'concluido', Cancelled: 'cancelado' }
...
export default class ConsolidatedInitialSchema extends BaseSchema {
  async up() {
    ...
    this.schema.alterTable('comments', (table) => {
      table.integer('parent_id').unsigned().references('id').inTable('comments').onDelete('SET NULL').nullable();
    ...
    this.schema.createTable('activity_logs', (table) => {
      ...
      table.integer('task_id').unsigned().references('id').inTable('tasks').onDelete('CASCADE').nullable();
  ...
  async down() {
    ...
    this.schema.alterTable('comments', (table) => {
      if (this.db.dialect.name !== 'sqlite3') { 
         try { table.dropForeign(['parent_id']) } catch (e) { console.warn('Could not drop parent_id FK from comments, it might not exist or have a different name.')}
      }
    ...
    this.schema.alterTable('occupations', (table) => {
      if (this.db.dialect.name !== 'sqlite3') {
        try { table.dropForeign(['user_id']) } catch (e) { console.warn('Could not drop user_id FK from occupations, it might not exist or have a different name.')}
      }
    ...
    this.schema.alterTable('roles', (table) => {
      if (this.db.dialect.name !== 'sqlite3') {
        try { table.dropForeign(['user_id']) } catch (e) { console.warn('Could not drop user_id FK from roles, it might not exist or have a different name.')}
      }
      table.dropColumn('user_id')
    })
