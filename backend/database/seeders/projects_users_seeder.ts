...
export default class ProjectsUsersSeeder extends BaseSeeder {
  async run() {
    try {
      ...
      console.log(
        'Associando usuários ao projeto Sistema de Gerenciamento de Tarefas (ID:',
        taskManagement.id,
        ')'
      )
      await taskManagement.related('users').attach([admin.id, manager.id, developer.id])

      console.log(
        'Associando usuários ao projeto Redesign do Site Institucional (ID:',
        websiteRedesign.id,
        ')'
      )
      await websiteRedesign.related('users').attach([manager.id, designer.id])

      console.log('Associando usuários ao projeto Aplicativo Mobile (ID:', mobileApp.id, ')')
      await mobileApp.related('users').attach([admin.id, manager.id, developer.id, designer.id])
