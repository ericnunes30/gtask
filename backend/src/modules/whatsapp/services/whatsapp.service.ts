...
import { WhatsAppConfig, WhatsAppMessage, WhatsAppResponse, WhatsAppUserPreferences, NotificationPriority } from '../interfaces/whatsapp.types';
...
export class WhatsAppService {
  ...
  async sendNotification(
    ...
    try {
      ...
      this.logger.log(`Full Notification: ${JSON.stringify(notification, null, 2)}`);
      ...
      this.logger.log(`User Priority Threshold: ${userWithWhatsApp.whatsappPriorityThreshold}`);
      this.logger.log(`User Quiet Hours Start: ${userWithWhatsApp.whatsappQuietHoursStart}`);
      this.logger.log(`User Quiet Hours End: ${userWithWhatsApp.whatsappQuietHoursEnd}`);
      ...
      if (!this.config.enabled) {
        ...
        return { success: false, error: 'WhatsApp notifications are disabled', timestamp: new Date() };
      }
      ...
      this.logger.log(`WhatsApp API Key Length: ${this.config.apiKey?.length || 0}`);
      this.logger.log(`WhatsApp API Key (first 10 chars): ${this.config.apiKey ? this.config.apiKey.substring(0, 10) + '...' : 'EMPTY'}`);
      this.logger.log(`WhatsApp Delay: ${this.config.delay}`);
      this.logger.log(`=== END CONFIG ===`);

      // Verificar se usuário está ativo
      if (!userWithWhatsApp.is_active) {
        this.logger.debug(`User ${userWithWhatsApp.id} is inactive, WhatsApp notifications disabled`);
        return { success: false, error: 'User is inactive', timestamp: new Date() };
      }

      // Verificar prioridade
      if (!this.shouldSendByPriority(notification.priority, userWithWhatsApp.whatsappPriorityThreshold)) {
        this.logger.debug(`Notification priority ${notification.priority} below threshold for user ${userWithWhatsApp.id}`);
        return { success: false, error: 'Priority below user threshold', timestamp: new Date() };
      }

      // Verificar horário de silêncio
      if (this.isQuietHours(userWithWhatsApp.whatsappQuietHoursStart, userWithWhatsApp.whatsappQuietHoursEnd)) {
        this.logger.debug(`Quiet hours active for user ${userWithWhatsApp.id}`);
        return { success: false, error: 'Quiet hours active', timestamp: new Date() };
      }

      // Verificar rate limit
      if (!this.checkRateLimit(userWithWhatsApp.id.toString())) {
        this.logger.warn(`Rate limit exceeded for user ${userWithWhatsApp.id}`);
        return { success: false, error: 'Rate limit exceeded', timestamp: new Date() };
      }
      ...
      if (!userWithWhatsApp.whatsapp) {
        ...
        return { success: false, error: 'No WhatsApp number configured', timestamp: new Date() };
      }

      const formattedPhone = this.messageFormatter.formatPhoneNumber(userWithWhatsApp.whatsapp);
      if (!this.messageFormatter.validatePhoneNumber(formattedPhone)) {
        this.logger.warn(`Invalid WhatsApp number for user ${userWithWhatsApp.id}: ${userWithWhatsApp.whatsapp}`);
        return { success: false, error: 'Invalid WhatsApp number format', timestamp: new Date() };
      }
      ...
      // Registrar métricas
      this.logger.log(`WhatsApp notification sent to user ${user.id}: ${notification.type}`);

      return result;
    } catch (error) {
      this.logger.error(`Failed to send WhatsApp notification to user ${user.id}:`, error);
      return {
        success: false,
        error: error.message,
        timestamp: new Date()
  ...
  async sendMessage(phoneNumber: string, text: string): Promise<WhatsAppResponse> {
  ...
  private async retryWithBackoff(phoneNumber: string, text: string, attempt: number): Promise<WhatsAppResponse> {
    if (attempt > 3) {
      this.logger.error(`Max retries exceeded for WhatsApp message to ${phoneNumber}`);
      return {
        success: false,
    ...
    this.logger.warn(`Retrying WhatsApp message to ${phoneNumber} (attempt ${attempt}) in ${delay}ms`);
  ...
  private shouldSendByPriority(notificationPriority: string, userThreshold: string): boolean {
    const priorityOrder = {
    ...
    const thresholdLevel = priorityOrder[userThreshold as NotificationPriority] || 2; // Default MEDIUM

    return notificationLevel >= thresholdLevel;
  }

  private isQuietHours(startTime: string | undefined, endTime: string | undefined): boolean {
