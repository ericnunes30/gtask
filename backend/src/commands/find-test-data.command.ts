...
@Command({ name: 'find:test-data', description: 'Encontra IDs de usuários, projetos e equipes para usar em testes.' })
export class FindTestDataCommand extends CommandRunner {
  ...
  async run(): Promise<void> {
    ...
    if (users.length > 0) {
      users.forEach(user => console.log(`ID: ${user.id}, Nome: ${user.name}, Email: ${user.email}`));
    } else {
    ...
    if (projects.length > 0) {
      projects.forEach(project => console.log(`ID: ${project.id}, Título: ${project.title}`));
    } else {
    ...
    if (occupations.length > 0) {
      occupations.forEach(occ => console.log(`ID: ${occ.id}, Nome: ${occ.name}`));
    } else {
