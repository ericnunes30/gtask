...
@Injectable()
@Command({ name: 'db:migrate:status', description: 'Show migration status - pending and executed' })
    ...
    try {
      ...
      try {
        ...
        if (pendingMigrations) {
          this.logger.log('  Run "npm run db:migrate" to execute pending migrations');
        }
