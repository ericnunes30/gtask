...
export class MigrateNotificationDataToNewFormat1757000000001 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const notification of notifications) {
            const newData = this.convertDataFormat(notification.type, notification.data);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        console.warn('Revertendo migração: Não é possível restaurar o formato antigo dos dados de notificação.');
    }
    ...
    private convertDataFormat(type: string, oldData: any): any {
        try {
            ...
            console.error(`Erro ao converter dados da notificação do tipo ${type}:`, error);
    ...
    private isNewFormat(data: any): boolean {
        ...
        ) && !(data.entityType !== undefined && data.entityId !== undefined); // Mas não o formato antigo
    }

    private convertTaskCreatedData(oldData: any): any {
        // Extrair informações do formato antigo
        const performerName = oldData.context?.performer?.name || 'Usuário desconhecido';
        const taskTitle = oldData.changes?.task?.newValue?.title || 'Tarefa desconhecida';
        const projectTitle = oldData.relatedEntities?.find((e: any) => e.type === 'project')?.name;
    ...
    private convertTaskStatusUpdatedData(oldData: any): any {
        // Extrair informações do formato antigo
        const performerName = oldData.context?.performer?.name || 'Usuário desconhecido';
        const taskTitle = oldData.relatedEntities?.find((e: any) => e.type === 'task')?.name || 'Tarefa desconhecida';
    ...
    private convertCommentCreatedData(oldData: any): any {
        ...
        const commentSnippet = commentContent.length > 50 ? 
